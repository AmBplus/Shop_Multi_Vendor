...
**مدیریت تم‌های پیش‌فرض:**
- کاربر می‌تواند هر تم پیش‌فرض را **ویرایش** کند و آن را به عنوان تم اختصاصی خود ذخیره کند
- امکان **افزودن به علاقه‌مندی‌ها** برای هر تم (ستاره‌دار کردن)
- تم‌های علاقه‌مند در اول لیست نمایش داده می‌شوند
- هر کاربر می‌تواند **تم مجزا برای پنل ادمین** و **تم مجزا برای بخش کاربری** ذخیره کند
...
interface CustomTheme {
  name: string;
  scope: 'user-panel' | 'admin-panel' | 'global'; // جداگانه برای ادمین/کاربر
  colors: {
    primary: string;       // hex
  ...
  typography: {
    fontFamily: string;    // از لیست فونت‌های موجود
    borderRadius: 'sharp' | 'soft' | 'rounded';
  };
  isDark: boolean;
  isFavorite: boolean;     // علاقه‌مندی
}
```
...
│  ویرایشگر تم                                              │
├─────────────────────────────┬────────────────────────────┤
│  پنل ویرایش متغیرها         │  پیش‌نمایش کلی (Live)       │
│                             │                            │
│  رنگ اصلی (Primary):        │  [نمایش کامپوننت‌های        │
│  [████] #3B82F6             │   اصلی در تم جاری]         │
│  ╔══════════════════╗       │                            │
│  ║  پیش‌نمایش دکمه  ║       │  مثل: هدر، کارت محصول،    │
│  ║  [ثبت سفارش]    ║       │  دکمه، فرم، جدول ...       │
│  ╚══════════════════╝       │                            │
│                             │                            │
...
│  ...                        │                            │
├─────────────────────────────┴────────────────────────────┤
│  [💾 ذخیره تم] [⭐ علاقه‌مندی] [📤 خروجی JSON] [↩ ریست]  │
└──────────────────────────────────────────────────────────┘
```

**قوانین UI ویرایشگر تم:**
- به ازای **هر متغیر رنگی**: کادر Color Picker + پیش‌نمایش زنده کامپوننت مرتبط زیر همان متغیر
- **پیش‌نمایش زنده کلی** در سمت دیگر صفحه (Real-time CSS Variable update)
- هر متغیر رنگی با کادر دور خودش + پیش‌نمایش مستقیم زیرش نمایش داده می‌شود
- تغییرات بلافاصله در هر دو بخش (پیش‌نمایش مجزا + پیش‌نمایش کلی) منعکس می‌شوند
- Import/Export JSON
- ذخیره در localStorage + ارسال به سرور (برای کاربران لاگین)

### 2.4 فونت‌های پیش‌فرض سیستم

سیستم با مجموعه‌ای از فونت‌های فارسی از پیش نصب‌شده عرضه می‌شود:

| فونت | نوع | پیش‌فرض |
|------|-----|---------|
| Vazir | سنسریف فارسی | ✅ پیش‌فرض سیستم |
| Shabnam | سنسریف فارسی | – |
| Samim | سنسریف فارسی | – |
| Tanha | نمایشی فارسی | – |

**تنظیمات فونت:**
- امکان تغییر فونت برای **پنل ادمین** و **بخش کاربری** به صورت **مجزا**
- تنظیمات فونت در `user_preferences` و `admin_preferences` جداگانه ذخیره می‌شود
- انتخاب فونت در ویرایشگر تم در دسترس است

### 2.5 تم مجزا برای پنل ادمین و پنل کاربری

```
کاربر می‌تواند دو تم مستقل تعریف کند:
  - تم پنل کاربری (فروشگاه)  → ذخیره در: user_preferences.user_panel_theme
  - تم پنل ادمین             → ذخیره در: user_preferences.admin_panel_theme

هنگام ورود به هر بخش، تم مربوطه بارگذاری می‌شود.
```

### 2.6 رفع مشکل Flash of Wrong Theme (FOWT)
